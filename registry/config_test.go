...
func TestLoadInvalidRegistries(t *testing.T) {
	var (
		...
		regC registrytypes.Registry
		regD registrytypes.Registry
		err  error
	)
	...
	mirrorsA := []string{"https://mirror-a1.com", "https://mirror-a2.com"}
	if err := regA.AddMirror(mirrorsA[0]); err != nil {
		t.Fatal(err)
	}
	if err := regA.AddMirror(mirrorsA[1]); err != nil {
		t.Fatal(err)
	}
	...
	// incompatible with regA as a mirror can only serve one host
	if err := regB.AddMirror(mirrorsA[1]); err != nil {
		t.Fatal(err)
	}

	regC, err = registrytypes.NewRegistry("https://registry-a.com/but-with-prefix")
	if err != nil {
		t.Fatal(err)
	}
	// incompatible with regB as a registry host cannot simultaneously serve
	// as a mirror
	if err := regC.AddMirror("https://registry-b.com"); err != nil {
		t.Fatal(err)
	}
	// comptabile with regA as mirrors serve the same host
	if err := regC.AddMirror(mirrorsA[0]); err != nil {
		t.Fatal(err)
	}

	regD, err = registrytypes.NewRegistry("https://registry-d.com:7000")
	if err != nil {
		t.Fatal(err)
	}
	// invalid registry as an official host cannot serve as a mirror
	if err := regD.AddMirror("https://docker.io:1234"); err != nil {
		t.Fatal(err)
	}

	// regD is invalid
	_, err = newServiceConfig(
		ServiceOptions{
			Registries: []registrytypes.Registry{regD},
		})
	if err == nil {
		t.Fatal("loading of invalid registry should fail")
	}
	assert.Contains(t, err.Error(), "cannot be used (reserved host)")

	// regA + regB are incompatible
	_, err = newServiceConfig(
		ServiceOptions{
	...
	assert.Contains(t, err.Error(), "can only serve one registry host")

	// regA + regC are compatible
	_, err = newServiceConfig(
		ServiceOptions{
			Registries: []registrytypes.Registry{regA, regC},
		})
	if err != nil {
		t.Fatal(err)
	}

	// regB + regC are incompatible
	_, err = newServiceConfig(
		ServiceOptions{
			Registries: []registrytypes.Registry{regB, regC},
		})
	if err == nil {
		t.Fatal("loading of invalid registry should fail")
	}
	assert.Contains(t, err.Error(), "cannot simultaneously serve as a mirror for")
}
